...
function activate(context) {
    ...
    const historyManager = new HistoryManager_1.HistoryManager(); // HistoryManagerをインスタンス化
    // サイドバープロバイダーを登録
    const sidebarProvider = new SidebarProvider_1.SidebarProvider(context.extensionUri, contextManager, promptGenerator, responseParser, historyManager // SidebarProviderにもHistoryManagerを渡す
    );
    ...
    context.subscriptions.push(vscode.commands.registerCommand('llmBridge.setInputCharLimit', (limit) => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('llmBridge.generateSplitPromptPart', async (partIndex) => {
        ...
        try {
            ...
            vscode.window.showErrorMessage(`分割プロンプト生成に失敗: ${error instanceof Error ? error.message : String(error)}`);
    ...
    context.subscriptions.push(vscode.commands.registerCommand('llmBridge.applyResponse', async () => {
        ...
        if (result.status === 'pending') { // プレビュー中の場合
            ...
            if (result.switchModeTo) { // モード切り替えが最優先 (成功時のみ)
                ...
                vscode.window.showInformationMessage(`LLMがモードを「${result.switchModeTo === 'browse' ? '閲覧' : '編集'}」に切り替えることを要求しました。${result.requestReason ? `理由: ${result.requestReason}` : ''}新しいプロンプトをクリップボードにコピーしました。LLMにペーストしてください。`);
                ...
                const newPrompt = await promptGenerator.generate();
                await vscode.env.clipboard.writeText(newPrompt);
                vscode.window.showInformationMessage(`LLMが${result.requestedFiles.length}個のファイルをリクエストしました。コンテキストに追加し、新しいプロンプトをクリップボードにコピーしました。LLMにペーストしてください。`);
                outputChannel.appendLine(`[${new Date().toLocaleTimeString()}] 新しいプロンプトをクリップボードにコピーしました。`);
                console.log(`[LLM Bridge] 新しいプロンプトをクリップボードにコピーしました。`);
                if (contextManager.getMode() === 'browse') { // このヒントはファイルリクエストのみの場合に意味がある
                    vscode.window.showInformationMessage('現在のモードは閲覧モードです。ファイル全文が必要な場合は、LLMの応答に <<<SWITCH_MODE: edit>>> も含めてリクエストしてください。');
                ...
                vscode.window.showInformationMessage(`LLMが応答の続きを要求しています。LLMに「続けて」と入力するか、再度プロンプトを生成してペーストしてください。`);
