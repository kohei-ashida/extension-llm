...
export type TaskType = 'general' | 'bugfix' | 'refactor' | 'feature' | 'review';
...
export class PromptGenerator {
     ...
     * 利用可能なタスク種別を取得
     */
    getAvailableTaskTypes(): { id: TaskType; name: string }[] {
     ...
     * 分割されたプロンプトを生成
     */
    async generateSplit(charLimit: number): Promise<string[]> {
     ...
     * ファイルブロックを分割
     */
    private splitFileBlocks(filesPart: string): string[] {
        const blocks: string[] = [];
